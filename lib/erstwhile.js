...
const validateField = (fieldDefinition, field) => {
  ...
  if((field == undefined || field == '' || (field.hasOwnProperty('trim')) && field.trim() != '') && fieldDefinition['default'] === undefined && fieldDefinition.required === true) {
    ...
    let validateSpecificField = (localField) => { 
      switch(fieldDefinition.type.toLowerCase()) {
        ...
        case 'time':
          ...
          break;
        case 'datetime':
          if(!validator.isISO8601(localField + '') && (localField.toISOString() && !validator.isISO8601(localField.toISOString()))) {
            retval.success = false;
            retval.error = `Field should be a properly formated date/time value.`;
          }
          break;
        case "boolean":
          if(localField !== true && localField !== false) {
            retval.success = false;
            retval.error = `Field should be a boolean.`;
          }
      }
    }
...
const validateRequest = (req, initialValues = false) => {
  let retval = {success: false, authenticated: req[process.env.ERSTWHILE_SESSION_KEY || 'session']?.hasOwnProperty('user_id')} ;
  if(initialValues) {
    req.bod
  }
  if (!process.env.ERSTWHILE_DESCRIPTION_PATH) {
    ...
    try {
      const apiDescription = require(`${process.cwd()}/${process.env.ERSTWHILE_DESCRIPTION_PATH}`);
      ...
      if(fields === null) {
        ...
        if(fields.type) {
          ...
          } else if(fields.payload.fields) {
            localFields = fields.payload.fields;
          } else if(fields.payload.type == "object") {
            localFields = apiDescription.entities[fields.payload.entity].fields;
          }
          if(localFields) {
            let location = null;
            ...
            if(location) {
              ...
              if(localFields) {
                // merge in the initial data if needed
                if(initialValues) {
                  req[location] = {...initialValues, ...req[location]}
                }
                for(let i = 0; i < localFields.length; i++) {
                  let testResult = validateField(localFields[i], req[location][localFields[i].key]);
                  if(testResult.success == false) {
                    goodSoFar = false;
                    if(!retval.errorsObj) {
                      retval.errorsObj = {}
                    }
                    retval.errorsObj[localFields[i].key] = testResult.error;
                  }
                }
              }
              if(goodSoFar) {
      ...
      if(apiDescription.application?.sessionKey?.type) {
        switch(apiDescription.application.sessionKey.type) {
          case "erstwhile-custom-header":
            if(!req.get(apiDescription.application.sessionKey.header)) {
              req[process.env.ERSTWHILE_SESSION_KEY || 'session'] = {};
            } else {
              let result = await req.orm.findUnique(process.env.ERSTWHILE_SESSION_TABLE || "erstwhile_sessions", {where: { id: req.get(apiDescription.application.sessionKey.header)}});
              if(result) {
                const now = new Date();
                if(now < result.expires) {
                  ...
                  await req.orm.update(process.env.ERSTWHILE_SESSION_TABLE || "erstwhile_sessions", {
                    where: {
                      id: req.get(apiDescription.application.sessionKey.header)
                    },
                    data: {
                  ...
                  await req.orm.delete(process.env.ERSTWHILE_SESSION_TABLE || "erstwhile_sessions", {where: {id: req.get(apiDescription.application.sessionKey.header)}});
                  req[process.env.ERSTWHILE_SESSION_KEY || 'session'] = {};
                }
              } else {
                req[process.env.ERSTWHILE_SESSION_KEY || 'session'] = {};
